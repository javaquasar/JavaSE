...
import core.lecture2.control1.Matrix;

public class MatrixExemple {
	/*public static final void invert(double A[][]) {
		...
		if (A[0].length != n) {
			System.out
					.println("Error in Matrix.invert, inconsistent array sizes.");
		}
		...
		for (int k = 0; k < n; k++) {
			...
			// перестановка к-ой строки и к-ого столбца с стобцом и строкой,
			// содержащий основной элемент(pivot основу)
			hold = row[k];
			row[k] = row[I_pivot];
			row[I_pivot] = hold;
			hold = col[k];
	...
	public static final double determinant(final double A[][]) {
		...
		for (int k = 0; k < n - 1; k++) {
			...
			// если нашлась такая основа, то меняем знак определителя и меняем
			// местами столбцы
			if (I_pivot != k) {
				hold = row[k];
				row[k] = row[I_pivot];
	...
	public static final void add(final double A[][], final double B[][],
		...
		int ni = A.length;
		int nj = A[0].length;
		if (B.length != ni || C.length != ni || B[0].length != nj
				|| C[0].length != nj) {
	...
	public static final void subtract(final double A[][], final double B[][],
			double C[][]) {
		int ni = A.length;
		int nj = A[0].length;
		if (B.length != ni || C.length != ni || B[0].length != nj
	...
	private final Matrix invert(double A[][]) {
        int n = A.length;
        int row[] = new int[n];
        int col[] = new int[n];
        double temp[] = new double[n];
	...
	public static Matrix Inverse_matrix_jordan_gauss(Matrix A) {
		int i, j, k;
		int size = A.RowCount;
		...
		for (k = 0; k < size; k++) {
			for (j = k + 1; j < size; j++) {
				A.mat[k][j] = A.mat[k][j] / A.mat[k][k];// все элементы k-ой
														// строки матрицы A,
														// кроме k-ого и до
														// него, делим на
														// разрешающий элемент -
			...
			for (j = 0; j < size; j++) {
				E.mat[k][j] = E.mat[k][j] / A.mat[k][k];// все элементы k-ой
														// строки матрицы e,
														// делим на разрешающий
														// элемент - a[k][k]
			}
			A.mat[k][k] = A.mat[k][k] / A.mat[k][k];// элемент соответствующий
													// разрещающему - делим на
													// самого себя(т.е получит.
													// 1 )
			// идём сверху вниз, обходя k-ую строку
			if (k > 0) {// если номер ведущей строки не первый
				for (i = 0; i < k; i++) { // строки, находящиеся выше k-ой
					for (j = 0; j < size; j++) {
						E.mat[i][j] = E.mat[i][j] - E.mat[k][j] * A.mat[i][k];// Вычисляем
																				// элементы
																				// матрицы
																				// e,идя
																				// по
					...
					for (j = size - 1; j >= k; j--) {
						A.mat[i][j] = A.mat[i][j] - A.mat[k][j] * A.mat[i][k];// Вычисляем
																				// элементы
																				// матрицы
																				// a,идя
																				// по
		...
		return E;// На месте исходной матрицы должна получиться единичная а на
					// месте единичной - обратная.
	}*/
}
