package com.javaquasar.java.core.chapter_11_String.practice;
 ...
 */
public class CopyOfWorkWithfString {
	...
	public static void main(String[] args) {
		...
		case 7:
			...
			System.out.print("Введите строку -> ");
			string = "ма ма мыk ла раму 1 2345 67 890 12345 67 890";// scanner.nextLine();
			System.out.println(formatInWidth(string, 10));
			break;
		case 8:
	...
	public static String correct(String string) {
		...
		for (int i = 0; i < wordArray.length; i++) {
			...
			for (int j = 0; j < wordArray[i].length(); j++) {
				...
				if (j == 0 && i > 0 && (Character.isUpperCase(ch))) {
					if (wordArray[i - 1].charAt(wordArray[i - 1].length() - 1) != '.') {
						sb.append(". " + ch);
					} else {
						sb.append(ch);
					}
	...
	/*
	 * while (st.hasMoreTokens()) { myString = st.nextToken(); myWidth +=
	 * myString.length() + 1; if (myWidth > width) { stringBuilder.append("\n" +
	 * myString + " "); System.out.println("myWidth" + myWidth); myWidth =
	 * myString.length() + 1; } else { stringBuilder.append(myString + " "); } }
	 */
