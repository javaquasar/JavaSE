...
public class ClientWindow {
	...
	public static void main(String[] args) throws Exception {
		final String name = JOptionPane.showInputDialog(null, "Введите имя:", "Имя пользователя", JOptionPane.PLAIN_MESSAGE);
	...
	private class ChatClient implements Runnable {
		...
		ChatClient(String userName, String serverName) throws Exception { // создаем
																			// нового
																			// клиента
																			// с
																			// указанным
			...
			socketClient = new Socket(serverName, 9000);// создаем сокет (хост
														// localhost и порт
														// №9999)
			...
			new Thread(this).start(); // создаем и запускаем нить чтения из
										// textAreaMessages
		}
		...
		synchronized void close() {// метод close синхронизирован, чтобы
									// исключить двойное закрытие.
